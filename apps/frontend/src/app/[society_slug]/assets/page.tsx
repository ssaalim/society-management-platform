      ...
      <div className="w-full max-w-[1600px] mx-auto space-y-3.5 z-10">
        ...
        <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Asset Management Center</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Track warranties, AMC contracts, schedule servicing & audit maintenance costs</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          {[
            { label: 'Total Assets', value: String(assetsList.length), icon: Box, color: 'text-indigo-600 dark:text-indigo-400' },
            { label: 'Total Capital Value', value: `₹${(totalAssetValue / 100000).toFixed(1)}L`, icon: Coins, color: 'text-emerald-600 dark:text-emerald-400' },
            { label: 'Annual AMC Cost', value: `₹${(totalAmcCost).toLocaleString('en-IN')}`, icon: Calendar, color: 'text-amber-600 dark:text-amber-400' },
            { label: 'Service Due (30 days)', value: String(nearServiceAssets), icon: AlertTriangle, color: nearServiceAssets > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-500' },
          ].map((kpi, i) => (
            <div key={i} className="border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-950/40 p-3 space-y-1 shadow-xs">
        ...
        {message && (
          <div className={`rounded-xl border p-3 text-xs font-semibold flex items-center gap-2.5 shadow-xs ${
          ...
          <>
            ...
            {activeTab === 'cost' && (
              <div className="space-y-3.5">
                <div className="bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 p-3 rounded-xl shadow-xs">
                  <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">Maintenance Cost Ledger Analysis</h3>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Aggregated repair and servicing expenditures grouped by asset and work type. Log entries via the Maintenance button on each asset card.</p>
                </div>

                {costAnalysis.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-2 py-16 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl text-slate-500">
                    <Wrench className="h-8 w-8 opacity-20" />
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">No maintenance records yet</p>
                    <p className="text-xs text-center">Go to <span className="text-indigo-600 dark:text-indigo-400 font-bold">Assets Roster</span> and click <span className="text-indigo-600 dark:text-indigo-400 font-bold">"Log Maintenance"</span> on any asset to record a cost entry.</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left text-xs border-collapse whitespace-nowrap">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-black/60 text-slate-700 dark:text-slate-400 font-semibold border-b border-slate-200 dark:border-slate-800 text-[10px] uppercase">
                          ...
                          <th className="px-3.5 py-2.5">Work Type</th>
                          <th className="px-3.5 py-2.5 text-right">Total Expenditure (₹)</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40">
                        {costAnalysis.map((row, idx) => (
                          <tr key={idx} className="text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900/20 transition-colors">
                            <td className="px-3.5 py-2.5 font-bold text-slate-900 dark:text-slate-200">{row.assetName}</td>
                            <td className="px-3.5 py-2.5">
                              <span className={`text-[10px] border rounded-full px-2 py-0.5 font-bold uppercase ${row.type === 'MAINTENANCE' ? 'border-indigo-200 dark:border-indigo-900/50 bg-indigo-50 dark:bg-indigo-950/30 text-indigo-700 dark:text-indigo-400' : 'border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400'}`}>
                                {row.type}
                              </span>
                            </td>
                        ...
                        <tr className="bg-slate-50 dark:bg-slate-950/60 font-bold border-t-2 border-slate-200 dark:border-slate-700">
                          <td className="px-3.5 py-2.5 text-slate-900 dark:text-slate-300" colSpan={2}>Total Maintenance & Repair Spend</td>
                          <td className="px-3.5 py-2.5 text-right font-mono text-indigo-600 dark:text-indigo-400 font-black text-sm">
