...
import {
  ...
  Building, Receipt, FileText, Scale, Coins, PieChart
} from 'lucide-react';
...
function StatCard({ label, value, sub, icon: Icon, color, trend }: {
  ...
  return (
    <div className={`relative p-4 rounded-xl border bg-slate-950/40 space-y-2 overflow-hidden group hover:border-slate-700 transition-all ${color.replace('text-', 'border-').replace('-400', '-900/50')}`}>
...
export default function ReportsCenterPage() {
  ...
  const STATUS_BG: Record<string, string> = {
    PAID: 'bg-emerald-950/40 border-emerald-900/40',
    UNPAID: 'bg-red-950/40 border-red-900/40',
    PARTIAL: 'bg-amber-950/40 border-amber-900/40',
    OPEN: 'bg-amber-950/40 border-amber-900/40',
    ASSIGNED: 'bg-indigo-950/40 border-indigo-900/40',
    RESOLVED: 'bg-emerald-950/40 border-emerald-900/40',
    CLOSED: 'bg-slate-900/40 border-slate-800/40',
    LOW: 'bg-slate-900/40 border-slate-800/40',
    MEDIUM: 'bg-amber-950/40 border-amber-900/40',
    HIGH: 'bg-orange-950/40 border-orange-900/40',
    URGENT: 'bg-rose-950/40 border-rose-900/40',
  };
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Society financial intelligence, occupancy, complaints & asset insights</p>
            </div>
          </div>
          <button
            onClick={() => fetchAll()}
            disabled={isRefreshing}
            className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-indigo-400 border border-slate-800 hover:border-indigo-900 rounded-lg px-3 py-2 transition-all"
          >
        ...
        <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-hide">
          {TABS.map((tab) => (
            <button
              ...
              className={`flex items-center gap-1.5 whitespace-nowrap px-3.5 py-2 rounded-lg text-xs font-semibold transition-all ${
                activeTab === tab.id
        ...
        {isLoading ? (
          ...
          <>
            ...
            {activeTab === 'complaints' && (
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="border border-slate-800 rounded-xl bg-slate-950/20 p-5 space-y-3">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">By Status</h3>
                    {complaintsData.byStatus.map((row: any, i: number) => (
                      <div key={i} className={`flex justify-between items-center p-2.5 rounded-lg border text-xs ${STATUS_BG[row.status] || 'bg-slate-900/40 border-slate-800'}`}>
                        <span className={`font-bold ${STATUS_COLORS[row.status] || 'text-slate-400'}`}>{row.status}</span>
                        <span className="font-bold text-slate-200">{row.count} complaints</span>
                      </div>
                    ))}
                  ...
                  <div className="border border-slate-800 rounded-xl bg-slate-950/20 p-5 space-y-3">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">By Priority</h3>
                    {complaintsData.byPriority.map((row: any, i: number) => (
                      <div key={i} className={`flex justify-between items-center p-2.5 rounded-lg border text-xs ${STATUS_BG[row.priority] || 'bg-slate-900/40 border-slate-800'}`}>
                        <span className={`font-bold ${STATUS_COLORS[row.priority] || 'text-slate-400'}`}>{row.priority}</span>
                        <span className="font-bold text-slate-200">{row.count} complaints</span>
                      </div>
                    ))}
            ...
            {activeTab === 'assets' && (
              <div className="space-y-5">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <StatCard label="Total Assets" value={String(assetData?.summary?.totalAssets || 0)} icon={Package} color="text-violet-400" />
                  <StatCard label="Purchase Value" value={`₹${fmt(assetData?.summary?.totalValue || 0)}`} sub="Original cost" icon={Coins} color="text-indigo-400" />
                  <StatCard label="Depreciation" value={`₹${fmt(assetData?.summary?.totalDepreciation || 0)}`} sub="Cumulative to date" icon={ArrowDownRight} color="text-amber-400" />
                  <StatCard label="Net Book Value" value={`₹${fmt(assetData?.summary?.netBookValue || 0)}`} sub="After depreciation" icon={Scale} color="text-emerald-400" />
                </div>

                <div className="border border-slate-800 rounded-xl bg-slate-950/20 p-5 space-y-3">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
                    <Package className="h-4 w-4 text-violet-400" /> Asset Register By Category
                  </h3>
                  ...
                  {assetData.breakdown.map((row: any, i: number) => (
                    <div key={i} className="space-y-1 p-3 rounded-lg bg-slate-900/60 border border-slate-800">
                      <div className="flex justify-between text-xs">
                        <span className="font-bold text-slate-200">{row.category || 'Uncategorized'} <span className="text-slate-500 font-normal">({row.count} items)</span></span>
                        <span className="text-violet-400 font-mono font-bold">₹{fmt(row.totalValue)}</span>
                      </div>
            ...
            {activeTab === 'latefee' && (
              <div className="space-y-5">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <StatCard label="Late Fees Collected" value={`₹${fmt(lateFeeData?.totalLateFeeCollected || 0)}`} sub="Penalty income received" icon={Clock} color="text-rose-400" />
                  <StatCard label="Late Fees Waived" value={`₹${fmt(lateFeeData?.totalLateFeeWaived || 0)}`} sub="Society concessions granted" icon={Tag} color="text-amber-400" />
                  <StatCard label="Discounts Granted" value={`₹${fmt(lateFeeData?.totalDiscountsGranted || 0)}`} sub="Custom rebates applied" icon={Tag} color="text-indigo-400" />
                  <StatCard label="Waivers Granted" value={String(lateFeeData?.waiversCount || 0)} sub={`of ${lateFeeData?.receiptCount || 0} total receipts`} icon={CheckCircle} color="text-violet-400" />
                </div>

                <div className="border border-slate-800 rounded-xl bg-slate-950/20 p-5 space-y-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
                    <Clock className="h-4 w-4 text-rose-400" /> Late Fee Summary
                  </h3>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      { label: 'Total Late Fee Collected (Penalty Income)', value: `₹${fmt(lateFeeData?.totalLateFeeCollected || 0)}`, color: 'text-rose-400', bg: 'bg-rose-950/30 border-rose-900/40' },
                      { label: 'Total Late Fee Waived (Society Forgiven)', value: `₹${fmt(lateFeeData?.totalLateFeeWaived || 0)}`, color: 'text-amber-400', bg: 'bg-amber-950/30 border-amber-900/40' },
                      { label: 'Total Discounts Granted (Concessions)', value: `₹${fmt(lateFeeData?.totalDiscountsGranted || 0)}`, color: 'text-indigo-400', bg: 'bg-indigo-950/30 border-indigo-900/40' },
                    ].map((item, i) => (
                      <div key={i} className={`p-4 rounded-xl border ${item.bg} text-center space-y-1`}>
                  ...
                  <p className="text-[11px] text-slate-500 border-t border-slate-800 pt-3">
                    Configure late fee policy in <span className="text-indigo-400 font-semibold">Maintenance → Formula & Settings → Late Fees & Overdue Penalty Policy</span>. Waivers and discounts can be applied per receipt when recording payments.
                  </p>
                </div>
              </div>
            )}
