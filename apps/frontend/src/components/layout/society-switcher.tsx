...
export const SocietySwitcher: React.FC = () => {
  ...
  if (memberships.length === 1) {
    ...
    return (
      <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-lg border border-slate-800 bg-slate-950/40 text-slate-300">
        <Building className="h-4 w-4 text-indigo-400 shrink-0" />
        <div className="text-left">
          <p className="text-xs font-bold truncate max-w-[150px] text-slate-100 flex items-center gap-1.5">
            {single.societyName}
            {single.isExpired && (
              <span className="text-[9px] px-1.5 py-0.2 rounded bg-red-950 text-red-400 border border-red-900/60 font-semibold">
                Expired
              </span>
            )}
          </p>
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={`society-switcher-btn flex items-center justify-between gap-3 px-3 py-1.5 rounded-lg border text-left transition-all focus:outline-none ${
          activeSociety?.isExpired
            ? 'border-red-900/60 bg-red-950/30 text-red-300'
            : 'border-slate-800 bg-slate-950/60 hover:bg-slate-900/80 text-slate-200'
        }`}
      >
        <div className="flex items-center gap-2.5">
          <Building className={`h-4 w-4 shrink-0 ${activeSociety?.isExpired ? 'text-red-400' : 'text-indigo-400'}`} />
          <div>
            <div className="flex items-center gap-1.5">
              <p className="text-xs font-bold truncate max-w-[150px] text-slate-100">
                {activeSociety?.societyName || 'Select Society'}
              </p>
              {activeSociety?.societyId === defaultSocietyId && (
                <Star className="h-3 w-3 text-amber-400 fill-amber-400 shrink-0" title="Default Society" />
              )}
            </div>
      ...
      {isOpen && (
        <>
          ...
          <div className="society-switcher-dropdown absolute left-0 mt-2 w-80 rounded-xl border border-slate-800 bg-slate-950 p-2 shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              ...
              {memberships.map((m) => {
                ...
                return (
                  <div
                    ...
                    className={`society-switcher-option p-2.5 rounded-lg border transition-all flex items-center justify-between gap-2 ${
                      isExpired
                        ? 'opacity-60 border-red-900/30 bg-red-950/10 cursor-not-allowed'
                        : isActive
                        ? 'bg-indigo-950/50 border-indigo-500/50 text-indigo-200 cursor-pointer'
                        : 'border-transparent hover:bg-slate-900 hover:border-slate-800 text-slate-300 cursor-pointer'
                    }`}
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {isDefault && (
                          <span className="inline-flex items-center gap-0.5 text-[9px] bg-amber-950/60 text-amber-400 border border-amber-900/60 px-1.5 py-0.2 rounded-full font-bold">
            ...
            <div className="p-2 border-t border-slate-800/60 mt-1.5 text-[10px] text-slate-500 text-center">
              💡 Click <Star className="h-3 w-3 text-amber-400 inline" /> to change your default login society.
            </div>
          </div>
        </>
      )}
